...
class Library {
    ...
    func searchByTitle(_ title:String) -> String {
        ...
        for (k,v) in catalogue {
            if (v.title == title) && !checkedOutBooks.keys.contains(k) {
                return "Available"
            } else if (v.title == title) && checkedOutBooks.keys.contains(k) {
                return " Checked out by " + (checkedOutBooks[k]!.name)
            }
        }
        return "Not in catalouge"
    }
    ...
    func checkOut(_ bookId:String, _ person:Person) -> String {
        ...
        } else if !checkedOutBooks.keys.contains(bookId) && catalogue.keys.contains(bookId) {
    ...
    func checkIn(_ bookId:String) -> String {
        ...
        if !catalogue.keys.contains(bookId) {
            return "Book doesn't exist"
        } else if !checkedOutBooks.keys.contains(bookId) {
